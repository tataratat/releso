...
def check_if_color_is_supported_in_console() -> bool:
    """
    Checks if the connected consol has color support. Original from this gist
...
class SbSOvRLParserException(ValueError):
    """
    Parser Exception for the SbSOvRL package. Shows the context of the
    error. Also colors the output if coloring is available.
...
class SbSOvRLAgentUnknownException(Exception):
    """
    Parser Exception for the SbSOvRL package. Thrown when an agent is unknown.
    Configurable logger.

    Args:
        agent (str):
            Name of the unknown agent.
        logger (str):
...
class SbSOvRLValidationNotSet(Exception):
    """
    Parser Exception for the SbSOvRL package. Thrown when a validation
