...
class EpisodeLogCallback(BaseCallback):
    ...
    def __init__(
        ...
        self.inter_episode_dicts: Optional[
            List[Dict[int, Union[List[float], int]]]] = None
    ...
    def _on_step(self) -> bool:
        ...
        if self.inter_episode_dicts is None:
            self.inter_episode_dicts: List[Dict[str,
        ...
        for idx, loc_vars, done, reward, info in zip(
                count(), self.inter_episode_dicts, self.locals["dones"],
                self.locals["rewards"], self.locals["infos"]):
            ...
            if done:
                ...
                reset_reason = \
                    None if "reset_reason" not in info \
        ...
        if any(
                [
                    episode % self.update_n_episodes == 0 for episode in
                    range(self.last_checked_episode, self.episodes)
                ]):
            self._export()
