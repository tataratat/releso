...
class SPORObject(BaseModel):
    ...
    reward_on_completion: float | None = None
    ...
    additional_observations: (
        ObservationDefinition
        | ObservationDefinitionMulti
        | list[ObservationDefinition | ObservationDefinitionMulti]
        | None
    ) = None
...
class SPORObjectExecutor(SPORObject):
    ...
    multi_processor: MultiProcessor | MPIClusterMultiProcessor | None = None
    ...
    def spor_com_interface_add(
        ...
        if step_dict["info"].get(self.name) and (
            value := step_dict["info"][self.name].get("reset_reason")
        ):
...
class SPORObjectPythonFunction(SPORObjectExecutor):
    ...
    _run_func: Any | None = PrivateAttr(default=None)
    #: logger which is used for the internalized python functions
    _run_logger: Any | None = PrivateAttr(default=None)
    #: persistent function data is not touched by releso
    _func_data: Any | None = PrivateAttr(default=None)
...
class SPORObjectExternalPythonFunction(SPORObjectPythonFunction):
    ...
    python_file_path: str | pathlib.Path
...
class SPORList(BaseModel):
    ...
    def get_observations(
        self,
    ) -> list[ObservationDefinition | ObservationDefinitionMulti] | None:
    ...
    def run(
        ...
        for step in self.steps:
            ...
            if done and not info.get("reset_reason"):
